...
//On modifie le comportement de l'envoi du formulaire
$('#envoyer').click((event) => { 
    ...
    for (element of [document.querySelector("#nom"), document.querySelector("#prenom")]){
        if (testNumeroPresent(element.value)){
            ...
            $("<p> Cet élément ne peut pas contenir de numéros.</p>").insertAfter(element);
        }
    }
    ...
    //On vérifie qu'il y ait un chiffre
    str = document.querySelector("#mdp").value;
    if (str != ""){
        if (str.length < 8){
            erreur = true;
            $('#mdp').css('borderColor', "red");
            $("<p> Le mot de passe doit au moins contenir 8 caractères</p>").insertAfter($('#mdp'));
        } 
        else {
            //On va vérifier pour chaque char du mdp à quelle catégorie il appartient
        ...
        if (str != document.querySelector("#verifMdp").value && document.querySelector("#verifMdp").value!="") {
            ...
            $("<p> Les deux mots de passe doivent être identiques.</p>").insertAfter($('#verifMdp'));
        }
    }
